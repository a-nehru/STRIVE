...
Envelope-based targets and the assessment circle are centered on `CENTER` (exported from `engine.js`) — **adaptive per patient**: the assessment captures the resting hand position at settle (clamped) as the work center, stores it as `profile.center`, and `GameBase.start()` loads it (default `{0, 0.25}` for older profiles), so limited-motion patients never have to travel to a preset spot and games always place targets around the same point the envelope was measured from. `CENTER` is mutated in place; `samplePos`/`fromCenter` read it live.
